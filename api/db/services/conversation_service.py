...
async def async_completion(tenant_id, chat_id, question, name="New session", session_id=None, stream=True, *, user_id, **kwargs):
    ...
    if stream:
        try:
            async for ans in async_chat(dia, msg, True, user_id=user_id, session_id=session_id, **kwargs):
        ...
        answer = None
        async for ans in async_chat(dia, msg, False, user_id=user_id, session_id=session_id, **kwargs):
...
async def async_iframe_completion(dialog_id, question, session_id=None, stream=True, *, user_id, **kwargs):
    ...
    if not session_id:
        session_id = get_uuid()
        conv = {"id": session_id, "dialog_id": dialog_id, "user_id": user_id, "message": [{"role": "assistant", "content": dia.prompt_config["prologue"], "created_at": time.time()}]}
