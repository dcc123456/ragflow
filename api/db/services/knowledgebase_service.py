...
class KnowledgebaseService(CommonService):
    ...
    def create_with_name(cls, *, name: str, tenant_id: str, parser_id: str | None = None, **kwargs):
        ...
        if not operator:
            return False, get_data_error_result(message="The user is not in the team.")
    ...
    def check_embedding(cls, kb_id, embd_id, n=5):
        ...
        return {
            ...
            "sampled": len(samples),
            "valid": len(eff_sims),
            "avg_cos_sim": round(float(np.mean(eff_sims)) if eff_sims else 0, 6),
            "min_cos_sim": round(float(np.min(eff_sims)) if eff_sims else 0, 6),
            "max_cos_sim": round(float(np.max(eff_sims)) if eff_sims else 0, 6),
            "match_mode": mode,
        }
