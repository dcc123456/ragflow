...
async def get(doc_id):
    try:
        ...
        data = await asyncio.to_thread(settings.STORAGE_IMPL.get, b, n, tenant_id)
...
async def download_attachment(attachment_id):
    try:
        ...
        data = await asyncio.to_thread(settings.STORAGE_IMPL.get, current_user.id, attachment_id, current_user.id)
        response = await make_response(data)
...
async def get_image(image_id):
    ...
    for i in range(len(settings.STORAGE_IMPL.conn)):
        try:
            byts = await asyncio.to_thread(settings.STORAGE_IMPL.conn[i].get_object, bkt, nm)
            if not bytes:
                continue
