    ...
    if llm:
        if "model_name" in llm:
            ...
            if req.get("llm_id") is not None:
                llm_name, llm_factory, llm__tenant_id = TenantLLMService.split_model_name_and_factory(req["llm_id"])
                if not TenantLLMService.query(tenant_id=llm__tenant_id, llm_name=llm_name, llm_factory=llm_factory, model_type="chat"):
