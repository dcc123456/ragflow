...
async def create_or_upload(tenant_id: str = None):
    ...
    try:
        if "multipart/form-data" in content_type:
            ...
            try:
            ...
            except InsufficientResourceError as resource_err:
                # Subscription/storage quota rejected the upload. Translate
                # to the same billing codes the dataset upload uses so the
                # frontend upgrade modal flow is identical.
                code = {
                    "subscription": RetCode.BILLING_SUBSCRIPTION_INVALID,
                ...
                return get_resource_insufficient_result(
                    code=code,
                    message=str(resource_err),
                    detail={} if code == RetCode.BILLING_SUBSCRIPTION_INVALID else (resource_err.detail or {}),
                )
