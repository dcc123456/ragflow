...
async def upload():
    ...
    try:
        ...
        for file_obj in file_objs:
            ...
            while settings.STORAGE_IMPL.obj_exist(last_folder.id, location, current_user.id):
                location += "_"
            blob = file_obj.read()
            ...
            settings.STORAGE_IMPL.put(last_folder.id, location, blob, current_user.id)
            file = {
                "id": get_uuid(),
                "parent_id": last_folder.id,
...
async def rm():
    ...
    def _delete_single_file(file):
        try:
            if file.location:
                settings.STORAGE_IMPL.rm(file.parent_id, file.location, current_user.id)
        except Exception:
...
async def move():
    ...
    try:
        ...
        def _move_entry_recursive(source_file_entry, dest_folder):
            ...
            while settings.STORAGE_IMPL.obj_exist(dest_folder.id, new_location, current_user.id):
                new_location += "_"

            try:
                settings.STORAGE_IMPL.move(old_parent_id, old_location, dest_folder.id, new_location, current_user.id)
