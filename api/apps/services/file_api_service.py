...
async def upload_file(tenant_id: str, pf_id: str, file_objs: list):
    ...
    for file_obj in file_objs:
        ...
        # Enforce subscription + storage quota before writing to storage.
        # Mirrors the dataset upload path in api/db/services/file_service.py
        # so the same billing codes (2003/2005/...) reach the frontend.
        if settings.BILLING_ENABLED:
            from api.utils.billing import (
                check_dynamic_resources,
                raise_dynamic_resource_error,
            )

            file_size_bytes = len(blob)
            check_ok, check_info = await thread_pool_exec(check_dynamic_resources, tenant_id=tenant_id, storage=file_size_bytes)
            if not check_ok:
    ...
    def _move_entry_recursive(source_file_entry, dest_folder_entry, override_name=None):
        ...
        if need_storage_move:
            ...
            while settings.STORAGE_IMPL.obj_exist(dest_folder_entry.id, new_location, source_file_entry.tenant_id):
                new_location += "_"
            try:
                moved = settings.STORAGE_IMPL.move(source_file_entry.parent_id, source_file_entry.location, dest_folder_entry.id, new_location, source_file_entry.tenant_id)
