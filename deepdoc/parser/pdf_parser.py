...
from deepdoc.vision import OCR, AscendLayoutRecognizer, LayoutRecognizer, Recognizer, TableStructureRecognizer, AscendLayoutRecognizer
...
class RAGFlowPdfParser:
    ...
    def _table_transformer_job(self, ZM):
        ...
        for i in range(len(tbcnt) - 1):  # for page
            ...
            for j, tb_items in enumerate(recos[tbcnt[i]: tbcnt[i + 1]]):  # for table
                ...
                self.tabls_cells.append((figs[j], cells))
            self.tb_cpns.extend(pg)

        def gather(kwd, fzy=10, ption=0.6):
            eles = Recognizer.sort_Y_firstly(
                [r for r in self.tb_cpns if re.match(kwd, r.get("label", ""))], fzy)
        ...
        clmns = sorted([r for r in self.tb_cpns if re.match(
            r"table column$", r.get("label", ""))], key=lambda x: (x["pn"], x["layoutno"], x["x0"]))
    ...
    def _extract_table_figure(self, need_image, ZM, return_html, need_position, separate_tables_figures=False):
        ...
        for k, bxs in tables.items():
            ...
            html = self.tbl_det.construct_table(bxs, is_english=self.is_english,
                                                    ocr_model=self.ocr, with_image=True,
