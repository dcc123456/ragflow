...
class EngineBuilder:
    ...
    def __build_engine(
        self, fp16: bool = True, input_shape: Union[List, Tuple] = (1, 3, 640, 640), iou_thres: float = 0.65, conf_thres: float = 0.25, topk: int = 100, with_profiling: bool = True
    ) -> None:
        ...
        if engine is not None:
            ...
            self.logger.log(trt.Logger.WARNING, f"Build TensorRT engine finished.\nSaved to {str(self.weight.absolute())}")
        else:
    ...
    def build_from_api(
        ...
        batched_nms = Detect(self.network, state_dict, input_tensors22, strides, "Detect.22", reg_max, fp16, iou_thres, conf_thres, topk)
        for o in range(batched_nms.num_outputs):
            self.network.mark_output(batched_nms.get_output(o))


class TRTModule(torch.nn.Module):
    dtypeMapping = {trt.bool: torch.bool, trt.int8: torch.int8, trt.int32: torch.int32, trt.float16: torch.float16, trt.float32: torch.float32}
