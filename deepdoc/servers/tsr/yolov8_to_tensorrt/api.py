...
def Conv2d(network: trt.INetworkDefinition, weights: OrderedDict, input: trt.ITensor, out_channel: int, ksize: int, stride: int, group: int, layer_name: str) -> trt.ILayer:
    padding = ksize // 2
...
def Conv(network: trt.INetworkDefinition, weights: OrderedDict, input: trt.ITensor, out_channel: int, ksize: int, stride: int, group: int, layer_name: str) -> trt.ILayer:
    padding = ksize // 2
    if ksize > 3:
        padding -= 1
...
def Bottleneck(network: trt.INetworkDefinition, weights: OrderedDict, input: trt.ITensor, c1: int, c2: int, shortcut: bool, group: int, scale: float, layer_name: str) -> trt.ILayer:
    c_ = int(c2 * scale)
...
def C2f(network: trt.INetworkDefinition, weights: OrderedDict, input: trt.ITensor, cout: int, n: int, shortcut: bool, group: int, scale: float, layer_name: str) -> trt.ILayer:
...
def SPPF(network: trt.INetworkDefinition, weights: OrderedDict, input: trt.ITensor, c1: int, c2: int, ksize: int, layer_name: str) -> trt.ILayer:
    c_ = c1 // 2
...
def Detect(
    ...
    batched_nms_op = plugin_creator.create_plugin(
        name="batched_nms", field_collection=trt.PluginFieldCollection([background_class, box_coding, iou_threshold, max_output_boxes, plugin_version, score_activation, score_threshold])
    )
