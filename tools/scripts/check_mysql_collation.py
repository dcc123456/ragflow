...
if __name__ == "__main__":
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute("SELECT DEFAULT_CHARACTER_SET_NAME, DEFAULT_COLLATION_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = %s", (database,))
            result = cursor.fetchone()
            print(f"database DEFAULT_CHARACTER_SET_NAME: {result[0]}, DEFAULT_COLLATION_NAME: {result[1]}")

            cursor.execute("SELECT TABLE_NAME, TABLE_COLLATION FROM information_schema.TABLES WHERE TABLE_SCHEMA = %s", (database,))
            collation_map = {}
