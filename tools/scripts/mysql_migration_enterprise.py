...
class TenantModelProviderStage(MigrationStage):
    ...
    def check(self) -> bool:
        ...
        extra_where = " AND t1.create_time > %s" if self.create_time else ""
    ...
    def execute(self) -> tuple[int, list]:
        ...
        if self.dry_run:
            # Dry-run mode: count records without storing them
            total_count = 0
            batch_size = 100
            while True:
            ...
            if total_count == 0:
                ...
                return 0, []
            logger.info(f"[DRY RUN] Would insert {total_count} records")
            return total_count, self.target_tables

        # Execute mode: fetch and insert in batches
        batch_size = 100
        batch_num = 0
        while True:
            batch = cursor.fetchmany(batch_size)
            ...
            logger.info(f"Inserted batch {batch_num}: {len(batch)} records")
...
class TenantModelInstanceStage(MigrationStage):
    ...
    def execute(self) -> tuple[int, list]:
        ...
        # Use streaming dedup to avoid loading all records into memory at once.
        # We stream cursor rows, compute canonical api_key for is_tools dedup,
        # and maintain a "seen" set of (provider_id, canonical_api_key) to skip duplicates.
        extra_where = " AND tl.create_time > %s" if self.create_time else ""
        params = [self.create_time] if self.create_time else []
        cursor = self.db.execute_sql(
        ...
        # Streaming dedup: iterate cursor row-by-row, dedup by (provider_id, canonical_api_key)
        seen = {}  # (provider_id, canonical_api_key) -> record tuple
        dup_count = 0
        insert_queue = []
        batch_size = 100

        for row in cursor:
            tenant_id, llm_factory, api_key, status, provider_id = row
            canonical = TenantModelInstanceStage._strip_is_tools_from_api_key(api_key)
            key = (provider_id, canonical)
            if key in seen:
                dup_count += 1
                continue
            seen[key] = row
            insert_queue.append(row)
            if len(insert_queue) >= batch_size:
                if not self.dry_run:
                    self._insert_instance_batch(insert_queue, current_ts)
                    rows_inserted += len(insert_queue)
                insert_queue = []
        ...
        logger.info(f"Streaming dedup: {dup_count} duplicates skipped, {total_records} unique records to migrate")
        ...
        if self.dry_run:
            sample_records = list(seen.values())[:5]
            logger.info(f"[DRY RUN] Would insert {total_records} records")
            for tenant_id, llm_factory, api_key, status, provider_id in sample_records:
                logger.info(f"  instance_name=default, provider_id={provider_id}, api_key=***")
            if total_records > 5:
                logger.info(f"  ... and {total_records - 5} more records")
            return total_records, self.target_tables

        return rows_inserted, self.target_tables

    def _insert_instance_batch(self, batch: list, current_ts: int):
        """Insert a batch of tenant_model_instance records"""
        values = []
...
class TenantModelStage(MigrationStage):
    ...
    def execute(self) -> tuple[int, list]:
        ...
        if self.dry_run:
            # Dry-run mode: accumulate count without storing all records
            total_count = 0
            sample_records = []
            batch_size = 100
            while True:
                ...
                if not batch:
                    break
                resolved = self._resolve_instance_ids(batch, instance_lookup)
                total_count += len(resolved)
                if len(sample_records) < 5:
                    sample_records.extend(resolved[: 5 - len(sample_records)])
            if total_count == 0:
                logger.info("No records to migrate")
                return 0, []
            logger.info(f"[DRY RUN] Would insert {total_count} records")
            for source_id, llm_name, provider_id, instance_id, model_type, status, api_key in sample_records:
                logger.info(f"  model_name={llm_name}, provider_id={provider_id}, instance_id={instance_id}, model_type={model_type}")
            if total_count > 5:
                logger.info(f"  ... and {total_count - 5} more records")
            return total_count, self.target_tables
        ...
        while True:
            ...
            batch_num += 1
            values = []
            for source_id, llm_name, provider_id, instance_id, model_type, status, api_key in resolved:
                record_id = self.generate_uuid()
            ...
            """
            self.db.execute_sql(insert_sql)
            rows_inserted += len(resolved)
            logger.info(f"Inserted batch {batch_num}: {len(resolved)} records")
    ...
    def _build_instance_lookup(self) -> dict:
        ...
        lookup = {}
        for instance_id, provider_id, api_key in cursor:
            canonical = TenantModelInstanceStage._strip_is_tools_from_api_key(api_key)
    ...
    def _resolve_instance_ids(records: list, instance_lookup: dict) -> list:
        ...
        for source_id, llm_name, provider_id, model_type, status, api_key in records:
            ...
            if instance_id:
                ...
                skipped += 1
                logger.warning(f"No matching instance for tenant_llm id={source_id}, provider_id={provider_id}, llm_name={llm_name}, canonical_api_key={canonical[:30]}...")
...
class ModelIdConfigStage(MigrationStage):
    ...
    def iter_string_change_batches(self):
        for table_name, column_name in self.existing_columns(self.string_columns):
            ...
            cursor = self.db.execute_sql(
                f"SELECT id, `{column_name}` FROM `{table_name}` WHERE `{column_name}` IS NOT NULL AND `{column_name}` != '' AND `{column_name}` LIKE %s{time_clause}",
                ("%@%",) + time_params,
            )
            batch_index = 0
            scanned_rows = 0
    ...
    def execute(self) -> tuple[int, list]:
        ...
        for logical_table_name in logical_table_order:
            ...
            like_filters = [f"(`{column_name}` IS NOT NULL AND `{column_name}` != '' AND `{column_name}` LIKE %s)" for column_name in selected_columns]
            time_clause, time_params = self._time_filter_clause()
            logger.info(
            ...
            cursor = self.db.execute_sql(
                f"SELECT id, {', '.join(f'`{column_name}`' for column_name in selected_columns)} FROM `{working_table_name}` WHERE ({' OR '.join(like_filters)}){time_clause}",
