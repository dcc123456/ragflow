...
class K8sServiceDiscovery:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "Make sure you're either running in a pod with service account, "
    ...
    def discover_elasticsearch(self) -> Dict[str, Any]:
        ...
        if not es_service:
            raise RuntimeError(
                f"Could not find Elasticsearch service in namespace {self.namespace}. "
                f"Tried: {', '.join(es_service_names)}"
            )
    ...
    def _get_es_password(self) -> str:
        ...
        raise RuntimeError(
            f"Could not find Elasticsearch password in namespace {self.namespace}. "
            f"Please specify --es-password or set ELASTIC_PASSWORD environment variable."
        )
    ...
    def discover_infinity(self) -> Dict[str, Any]:
        ...
        if not infinity_service:
            raise RuntimeError(
                f"Could not find Infinity service in namespace {self.namespace}. "
                f"Tried: {', '.join(infinity_service_names)}"
            )
...
class EStoInfinityMigrator:
    ...
    def scan_indices(self, patterns: List[str] = None) -> List[str]:
        ...
        all_indices = sorted([
            ...
            if not idx.startswith('.') and idx != 'kibana'
        ])
...
Examples:
  ...
  python migrate_es_to_infinity.py --auto-discover --dry-run
    ...
    parser.add_argument(
        "--auto-discover",
        action="store_true",
        help="Auto-discover ES and Infinity services from K8s cluster"
    )
    parser.add_argument(
        "--namespace",
        default=os.environ.get("RAGFLOW_NAMESPACE", "ragflow"),
        help="Kubernetes namespace (default: ragflow)"
    )
    
    # Manual ES configuration
    parser.add_argument(
        "--es-host",
        help="Elasticsearch host URL (e.g., http://es01:9200)"
    )
    parser.add_argument(
        "--es-user",
        default="elastic",
    ...
    parser.add_argument(
        "--infinity-uri",
        help="Infinity connection URI (e.g., infinity:23817)"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["ragflow_*"],
        help="ES index patterns to migrate (default: ragflow_*)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
