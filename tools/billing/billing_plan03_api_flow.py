...
Tests: Starter -> Pro upgrade via Stripe Customer Portal (not direct checkout).
...
def create_paid_subscription(
    ...
    *,
    ...
    """Create a paid subscription and return (subscription_dict, created_timestamp)."""
...
def run_flow(args: argparse.Namespace) -> None:
    """Execute PLAN-03: Starter → Pro upgrade via Stripe Customer Portal."""
    ...
    if not stripe_api_key.startswith("sk_test_"):
        raise FlowError("PLAN-03 automation requires a Stripe test-mode secret key")

    stripe.api_key = stripe_api_key
    stripe.api_version = stripe_api_version
    clock = stripe.test_helpers.TestClock.create(frozen_time=int(time.time()), name=f"ragflow-plan03-{uuid.uuid4().hex[:8]}")
    ...
    # Step 1: Move from Trial to Starter through the real app checkout flow.
    ...
    if not schedule.get("redirect_to"):
        raise FlowError(f"expected portal redirect for Starter, got: {schedule}")
    ...
    starter_subscription, since_start = create_paid_subscription(
        customer_id,
        tenant_id,
    ...
    checkout_completed_event = build_checkout_session_completed_event(
        ...
        created=int(checkout_session.get("created") or since_start),
        expires_at=int(checkout_session.get("expires_at") or (int(checkout_session.get("created") or since_start) + 86400)),
    )
    ...
    # Wait for new billing history entry (invoice) to appear
    ...
    # Identify the new invoice entry
    ...
    for row in history_after:
        ...
        if inv_id and inv_id not in previous_invoice_ids:
            ...
            break
    if not new_invoice:
        raise FlowError("no new invoice found after Starter upgrade")
    amount_val = float(new_invoice.get("amount", 0) or 0)
    if amount_val <= 0:
        raise FlowError(f"Starter upgrade should create a paid invoice, got amount={new_invoice.get('amount')}")
    if new_invoice.get("status") != "paid":
        raise FlowError(f"expected paid status for Starter invoice, got {new_invoice.get('status')}")
    if not new_invoice.get("invoice_id"):
        raise FlowError("Starter invoice missing invoice_id in billing history")
    ...
    # Step 2: Initiate upgrade from Starter to Pro via pricing page
    # The /billing/checkout endpoint should detect existing paid subscription
    # and redirect to Customer Portal (not direct checkout)
    ...
    # Step 3: Simulate completing the upgrade in Stripe Portal
    # In manual mode, we need to update the subscription via Stripe API directly
    # since we can't actually interact with the portal UI
    ...
    # Step 4: Sync webhook events to reflect the upgrade
    sync_webhooks(
        client,
        mode=args.webhook_mode,
        webhook_secret=webhook_secret,
    ...
    # Step 5: Wait for plan to switch to Pro
    wait_for_plan(client, "Pro", args.webhook_timeout_seconds)

    # Step 6: Verify billing overview shows Pro quota
    ...
    # Step 7: Verify billing history records the upgrade with a paid invoice
    # Wait for new history entry to appear
    history_after = wait_for_history_count(client, len(history_before_upgrade) + 1, args.webhook_timeout_seconds, "Pro upgrade invoice")
    if not history_after:
    ...
    for row in history_after:
        ...
        if inv_id and inv_id not in previous_invoice_ids:
            ...
            if amount_val > 0 and row.get("status") == "paid":
                ...
                break
    if not new_invoice:
        raise FlowError("no new positive paid invoice found after Pro upgrade")
    if not new_invoice.get("invoice_id"):
        ...
        json.dumps(
            {
                ...
                "customer_id": customer_id,
                "initial_subscription_id": initial_subscription_id,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run billing PLAN-03: Starter->Pro upgrade via Customer Portal.")
