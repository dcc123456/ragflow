...
import { Avatar, Badge, Card, MenuProps, Space } from 'antd';
...
const KnowledgeCard = ({ item, showPrivilegeModal }: IProps) => {
  ...
  const operateItems = useMemo(() => {
    ...
    if (hasManagePermissionPermission(item.operator_permission)) {
      if (hasOwnerPermission(item.operator_permission)) {
        items.push({
          type: 'divider',
        });
      }
      items.push({
        key: '2',
        onClick: showPrivilegeModal,
        label: <PrivilegeDropdown></PrivilegeDropdown>,
      });
    }

    return items;
  }, [item, showPrivilegeModal]);
  ...
  return (
    <Badge.Ribbon
      ...
      className={classNames(styles.ribbon, {
        [styles.hideRibbon]: item.operator_permission === Permission.Owner,
      })}
    >
      <Card className={styles.card} onClick={handleCardClick}>
        <div className={styles.container}>
          <div className={styles.content}>
            ...
            {hasManagePermissionPermission(item.operator_permission) && (
              <OperateDropdown
                deleteItem={removeKnowledge}
                items={operateItems}
                showDeleteContent
                showDeleteItems={hasOwnerPermission(item.operator_permission)}
