...
def admin_metrics_worker(interval=60):
    while True:
        ...
        try:
            rabbit_conf = settings.RABBIT_CONF or {}
            host = rabbit_conf.get("host") or os.environ.get("RABBITMQ_HOST", "rabbitmq")
            port = rabbit_conf.get("api_port") or os.environ.get("RABBITMQ_API_PORT") or os.environ.get("RABBITMQ_MANAGEMENT_PORT", "15672")
            user = rabbit_conf.get("user") or os.environ.get("RABBITMQ_DEFAULT_USER", "ragflow")
            pwd = rabbit_conf.get("password") or os.environ.get("RABBITMQ_DEFAULT_PASS", "")
