...
class SMTPValidator:
    ...
    def validate_smtp_connection(
        ...
        try:
            ...
            # check params
            validation_error = SMTPValidator._validate_params(
                host, port, username, password
            )
            if validation_error:
                return {
                    "success": False,
                    "message": f"Check failed: {validation_error}"
                }

            # choose method
            if use_ssl:
                return SMTPValidator._validate_with_ssl(
                    host, port, username, password, timeout
                )
            elif use_tls:
                return SMTPValidator._validate_with_tls(
                    host, port, username, password, timeout
                )
            else:
                return SMTPValidator._validate_without_encryption(
                    host, port, username, password, timeout
                )
    ...
    @staticmethod
    def _validate_without_encryption(
            host: str, port: int, username: str, password: str, timeout: int
    ) -> Dict[str, any]:
        ...
        try:
            with smtplib.SMTP(
                ...
                return {
                    "success": True,
                    "message": "SMTP connection validated successfully(unencrypted)."
                }
    ...
    async def validate_async(
        ...
        try:
            ...
            result = await loop.run_in_executor(
                AsyncSMTPValidator._executor,
                SMTPValidator.validate_smtp_connection,
                host, port, username, password, use_tls, use_ssl, mail_timeout
            )
            ...
            return {
                "success": False,
                "message": f"Timeout after {mail_timeout} seconds.",
            ...
            return {
                "success": False,
                "message": f"Check SMTP connection exception: {str(e)}",
                "timestamp": datetime.now().isoformat()
            }
