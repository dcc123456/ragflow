...
async def run_document_structure_compile(handler, embedding_model: LLMBundle) -> None:
    ...
    for idx, (template_id, parser_cfg) in enumerate(active_templates):
        ...
        if synthesis_cfg.get("enabled"):
            ...
            if ctx.has_canceled_func(ctx.id):
                raise TaskCanceledException(f"Task {ctx.id} was cancelled before synthesis PLAN")
            ...
            if not example:
                ...
                try:
                    ...
                    if ctx.has_canceled_func(ctx.id):
                        raise TaskCanceledException(f"Task {ctx.id} was cancelled after synthesis PLAN")
                    ...
                    logging.exception(
                        "synthesis: failed for template %s",
                        template_id,
                    )
