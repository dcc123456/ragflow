...
class SyncBase:
    ...
    async def _run_sync_task_logic(self, task: dict):
        ...
        def handle_batch(document_batch):
            nonlocal added_docs, failed_docs, next_update, updated_docs
            ...
            try:
                if task.get("folder_name"):
                    err, dids = SyncLogsService.duplicate_and_parse(None, docs, task["tenant_id"], f"{self.SOURCE_NAME}/{task['connector_id']}", task["auto_parse"], task["folder_id"])
                else:
