...
class ShadowWriteProxy(DocStoreConnection):
    ...
    def __init__(self, primary_conn: DocStoreConnection, shadow_conns: list[DocStoreConnection]):
        ...
        if self.shadows:
            ...
            self.logger.info(
                f"ShadowWriteProxy initialized with primary={primary_conn.db_type()}, "
                f"shadows={shadow_types}"
            )
        else:
            self.logger.info(f"ShadowWriteProxy initialized with primary={primary_conn.db_type()}, no shadows")
    
    def _execute_on_shadow(self, shadow_idx: int, shadow_conn: DocStoreConnection, 
                          method: Callable, method_name: str, args: tuple, kwargs: dict) -> tuple[int, Any, Exception | None]:
        """
    ...
    def _compare_shadows_async(self, method_name: str, primary_result: Any, 
                                args: tuple, kwargs: dict, start_time: float):
        """
        ...
        for idx, shadow in enumerate(self.shadows):
            ...
            try:
                ...
                self.logger.warning(
                    f"[{_format_elapsed(time.time() - start_time)}] Shadow database {idx} ({shadow.db_type()}) error in {method_name}: "
                    f"{type(e).__name__}: {e}"
                )
    ...
    def _execute_with_shadows(self, method_name: str, *args, **kwargs) -> Any:
        ...
        _shadow_executor.submit(
            self._compare_shadows_async,
            method_name, primary_result, args, kwargs, start_time
        )
