...
def test_plan_01_full_lifecycle(billing_client):
    ...
    assert pro_period_end_after > pro_period_end_before_renewal, f"Pro billing cycle did not advance after renewal: before={pro_period_end_before_renewal}, after={pro_period_end_after}"
    ...
    assert overview_starter.get("resources", {}).get("apps", {}).get("limit", 0) == starter_quota, f"after downgrade to Starter, expected Starter apps quota {starter_quota}, got {overview_starter}"
...
def test_plan_05_starter_to_pro_payment_requirement(billing_client):
    ...
    assert requires_payment_method_setup, f"expected real billing API to require payment method setup when no reusable payment method exists, got response: {upgrade_response}"
    ...
    assert not overview_after.get("payment_required", False), f"unexpected payment_required=true before any upgrade invoice exists: {overview_after}"
    ...
    assert not failed_upgrade_response.get("requires_payment_method_setup"), (
        f"expected upgrade to use reusable failing card and enter pending_if_incomplete path, but API requested payment-method setup: {failed_upgrade_response}"
    )
    assert payment_state in {"pending", "requires_action"}, f"expected pending/attention payment_state from failed-card upgrade, got: {failed_upgrade_response}"
    ...
    assert payment_order.get("payment_status") == "failed", f"expected payment order to reach failed status, got {payment_order.get('payment_status')}"
    ...
    assert plan_after_failure == "Starter", f"expected Starter plan before payment recovery, got {plan_after_failure}: {current_after_failure}"
    ...
    assert apps_after_failure == starter_quota, f"expected Starter apps quota {starter_quota} before payment recovery, got {apps_after_failure}"
    ...
    assert invoice_dict.get("status") in valid_invoice_statuses, f"expected failed-card upgrade invoice to remain unpaid, got {invoice_dict}"
...
def test_plan_02_renewal_failure_recovery(billing_client):
    ...
    assert latest.get("status") == "paid", f"expected paid status for Pro upgrade invoice, got {latest.get('status')}"
    ...
    assert subscription_status in {"past_due", "incomplete", "unpaid", "incomplete_expired"}, f"expected delinquent status after renewal failure, got {subscription_status}: {current}"
    ...
    assert payment_required, f"expected payment_required=true in billing overview after renewal failure, got {overview_fail}"
    ...
    assert inv.get("status") in {"open", "uncollectible", "unpaid"}, f"expected renewal invoice to be unpaid/open, got status={inv.get('status')}: inv={inv.get('id')}"
    ...
    assert len(failed_rows) == 1, f"expected exactly one billing history row for failed invoice {renewal_invoice_id}, got {failed_rows}"
    failed_row = failed_rows[0]
    assert failed_row.get("status") == "unpaid", f"expected spend history to show unpaid for failed renewal invoice, got {failed_row}"
    assert float(failed_row.get("amount", 0) or 0) > 0, f"expected positive amount on failed renewal invoice row, got {failed_row}"
    ...
    assert len(history_after_pay) == history_count_after_failure, (
        f"renewal recovery should update the existing failed billing history row, not append a new row: before={history_count_after_failure}, after={len(history_after_pay)}"
    )
    paid_rows = [row for row in history_after_pay if row.get("invoice_id") == renewal_invoice_id]
    assert len(paid_rows) == 1, f"expected exactly one billing history row for recovered invoice {renewal_invoice_id}, got {paid_rows}"
    paid_row = paid_rows[0]
    assert paid_row.get("status") == "paid" and float(paid_row.get("amount", 0) or 0) > 0, f"paid invoice {renewal_invoice_id} not found in billing history after payment: history={history_after_pay}"
    assert payment_order_after_pay.get("payment_status") == "success", f"expected billing_payment_order to update the same invoice row from failed to success, got {payment_order_after_pay}"
    ...
    assert period_end_after > period_end_before_renewal, f"Pro billing cycle did not advance after renewal: before={period_end_before_renewal}, after={period_end_after}"
    final_plan = billing_client.wait_for_subscription_status("active")
    final_status = (final_plan.get("subscription_status") or "").lower()
    assert final_status == "active", f"expected subscription status 'active' after payment recovery, got {final_status}"

    overview_restored = billing_client.plan_overview()
    apps_restored = overview_restored.get("resources", {}).get("apps", {}).get("limit", 0)
    assert apps_restored == get_pro_quota_apps(), f"after payment, expected Pro quota, got {apps_restored}"
...
def test_plan_03_starter_to_pro_api_checkout(billing_client):
    ...
    assert overview_start.get("plan_name", "") == "Starter", f"expected plan_name 'Starter' after startup flow, got {overview_start.get('plan_name')}"
    starter_quota = get_starter_quota_apps()
    starter_apps_limit = overview_start.get("resources", {}).get("apps", {}).get("limit", 0)
    assert starter_apps_limit == starter_quota, f"expected Starter apps quota {starter_quota}, got {starter_apps_limit}"
    ...
    assert plan_name == "Pro", f"Upgrade to Pro failed: expected plan_name='Pro', got plan_name='{plan_name}'. Full response: {checkout_result}"
    ...
    assert apps_limit == get_pro_quota_apps(), f"expected Pro quota, got {apps_limit}"
