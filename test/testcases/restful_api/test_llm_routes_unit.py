...
def _load_llm_app(monkeypatch):
    ...
    tenant_llm_mod.get_enabled_tei_model = lambda: os.environ.get("TEI_MODEL")
    tenant_llm_mod.is_tei_enabled = lambda: "tei" in os.environ.get("COMPOSE_PROFILES", "")
    ...
    class _StubPermissionChangeLogService:
        @staticmethod
        def save(**_kwargs):
            return True
    ...
    class _StubPermissionService:
        ...
        @staticmethod
        def get_permissions_by_tenant_and_resource_id(**_kwargs):
            return []

    permission_service_mod.PermissionChangeLogService = _StubPermissionChangeLogService
    permission_service_mod.PermissionService = _StubPermissionService
    ...
    class _StubUserTenantService:
        @staticmethod
        def get_tenants_by_user_id(user_id):
            return [{"tenant_id": user_id, "nickname": "tenant"}]

        @staticmethod
        def filter_by_tenant_and_user_id(tenant_id, user_id):
            return SimpleNamespace(id="member-1", tenant_id=tenant_id, user_id=user_id)
    ...
    permission_utils_mod.has_permission_for_member = lambda **_kwargs: (True, None, 7)
...
def test_llm_list_app_grouping_availability_and_merge(monkeypatch):
    ...
    tenant_rows = [
        ...
        _TenantLLMRow(id=5, llm_name="tei-embed", llm_factory="Builtin", model_type="embedding", api_key="k5", status="1"),
    ]
