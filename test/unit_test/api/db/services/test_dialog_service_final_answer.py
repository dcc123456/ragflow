...
def _stub_doc_permission_filter(monkeypatch):
    """Keep final-answer unit tests isolated from DB-backed permission checks."""
    monkeypatch.setattr(
        dialog_service,
        "filter_accessible_doc_ids_for_user",
        lambda _user_id, _kb_ids, doc_ids: (doc_ids if doc_ids else ["doc-1"], ["tenant-1"], ""),
    )
