...
def _load_agent_api(monkeypatch, *, storage_get):
    ...
    _stub(
        monkeypatch, "api.utils.billing",
        check_dynamic_resources=lambda *_a, **_k: (True, None),
        get_dynamic_resource_error_result=lambda *_a, **_k: {"kind": "billing_error"},
    )
    _stub(
