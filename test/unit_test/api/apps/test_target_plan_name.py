...
def test_field_exists_on_model():
    ...
    assert hasattr(Subscription, "target_plan_name"), "Subscription model must have target_plan_name field"
...
def test_field_in_raw_subscription_fields():
    ...
    assert SubscriptionService.model.target_plan_name in SubscriptionService._RAW_SUBSCRIPTION_FIELDS, "_RAW_SUBSCRIPTION_FIELDS must include target_plan_name"
...
def test_migration_adds_target_plan_name():
    with open("api/db/db_models.py") as f:
        src = f.read()
    assert 'alter_db_add_column(migrator, "billing_subscription", "target_plan_name"' in src, "migrate_db() must call alter_db_add_column for billing_subscription.target_plan_name"
...
def test_webhook_clears_target_plan_name():
    ...
    assert "target_plan_name" in source and "plan_changed" in source, "subscription_dict must conditionally clear target_plan_name when plan changes"
...
def test_billing_app_writes_target_plan_name_on_downgrade():
    with open("api/apps/billing_app.py") as f:
        src = f.read()
    assert "target_plan_name=target_plan_name_for_downgrade" in src, "downgrade branch must write target_plan_name to DB"
...
def test_plan_downgrade_gated_by_plan_changed(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_: ({"num_storage_bytes": 10**6, "num_members": 1, "num_apps": 2}, {"quota_storage": 10**6, "quota_members": 1, "quota_apps": 5}),
    )
...
def test_storage_downgrade_fires_without_plan_changed(monkeypatch):
    ...
    monkeypatch.setattr(
        "api.services.downgrade_guard._load_tenant_data",
        lambda *_: ({"num_storage_bytes": 30 * 1000**3, "num_members": 1, "num_apps": 2}, {"quota_storage": 5 * 1000**3, "quota_members": 1, "quota_apps": 5}),
    )
    r = check_downgrade_effective_exceeded("t1", pre_sync, plan_changed=False)
