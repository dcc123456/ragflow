...
def _load_module(monkeypatch, *, tenant_model_records, factory_llm_infos=None):
    ...
    _stub(monkeypatch, "api.apps", current_user=SimpleNamespace(id="tenant-1"))
    _stub(monkeypatch, "api.db", ResourceType=SimpleNamespace(LLM="llm"), PermissionValue=SimpleNamespace(PERMISSION_READ="read"))
    _stub(monkeypatch, "api.utils.permission_utils", has_permission_for_member=lambda *a, **kw: (True, None))
    _stub(
        monkeypatch,
        "api.db.services.user_service",
        TenantService=SimpleNamespace(get_by_id=lambda tenant_id: (True, tenant), get_by_ids=lambda tenant_ids: [SimpleNamespace(id="tenant-1", name="tenant-1")]),
        UserTenantService=SimpleNamespace(get_tenants_by_user_id=lambda user_id: [], filter_by_tenant_and_user_id=lambda tenant_id, user_id: "member-1"),
    )
    _stub(
        monkeypatch,
