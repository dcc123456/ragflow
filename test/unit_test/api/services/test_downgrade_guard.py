...
def _product(**kw):
    d = {"name": "Trial", "quota_storage": 10 * 1000**3, "quota_members": 5, "quota_apps": 10, "priority": 1, "task_priority": "low"}
    d.update(kw)
    return d
...
class TestDailyScan:
    ...
    def test_warn_only_gt_72h(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "api.services.downgrade_guard._query_scheduled_downgrades",
            lambda: [sub],
        )
        monkeypatch.setattr(
