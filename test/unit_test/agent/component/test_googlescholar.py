...
def test_respects_top_n(monkeypatch):
    ...
    consumed = [0]
    monkeypatch.setattr(scholarly.scholarly, "search_pubs", lambda *a, **k: _fake_pubs(30, consumed))
...
def test_json_output_not_exhausted(monkeypatch):
    ...
    monkeypatch.setattr(scholarly.scholarly, "search_pubs", lambda *a, **k: _fake_pubs(30))
...
def test_empty_query_short_circuits(monkeypatch):
    ...
    monkeypatch.setattr(scholarly.scholarly, "search_pubs", spy)
    gs, _, out = _make_tool(top_n=5)
